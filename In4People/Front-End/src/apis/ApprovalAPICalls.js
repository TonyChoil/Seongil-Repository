...
import { GET_APPROVAL_SEARCHINFO, POST_APPROVAL_INSERT, GET_DETAIL} from '../modules/ModalModule';
...
export const callApprovalDetailAPI = (docCode) => {
    const requestURL = `http://${process.env.REACT_APP_RESTAPI_IP}:7777/api/v1/approval/detail/${docCode}`;

    console.log('ClickedDocCode', docCode);
    return async (dispatch, getState) => {
        ...
        const result = await fetch(requestURL, {
            ...
            .then(response => response.json());
        if (result.status === 200) {
            console.log('[ApprovalAPICalls] callGetApprovalDetailAPI RESULT : ', result);
...
export const callGetBookmarkAPI = ({ memCode }) => {
    let requestURL;


    requestURL = `http://${process.env.REACT_APP_RESTAPI_IP}:7777/api/v1/approval/bookmark/${memCode}`;
    ...
    return async (dispatch, getState) => {
        ...
        const result = await fetch(requestURL, {
            ...
            .then(response => response.json());
        if (result.status === 200) {
            console.log('[ApprovalAPICalls] callGetBookmarkAPI RESULT : ', result);
            dispatch({ type: GET_BOOKMARK, payload: result.data });
        }
    };
}
