...
//        return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "등록 성공", approvalService.postApprovalBookmark(bookmarkDTO)));
    ...
    @Operation(summary = "결재 북마크 목록", description = "북마크 문서리스트", tags = { "ApprovalController" })
    @GetMapping("/approval/bookmark/{memCode}")
    public ResponseEntity<ResponseDTO> getBookmarkList(@PathVariable String memCode){

        System.out.println("memCode ==== " + memCode);

        return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "조회 성공", approvalService.getBookmarkList(memCode)));
    }
    ...
    @GetMapping(value = "/approval/detail/{docCode}")
    public ResponseEntity<ResponseDTO> getApprovalDetail(@PathVariable String docCode){

        System.out.println("getApprovalDetail docCode ==================== = " + docCode);

        return ResponseEntity.ok().body(new ResponseDTO(HttpStatus.OK, "결재 상세 조회 완료", approvalService.getApporvalDetail(docCode)));
    }
}
