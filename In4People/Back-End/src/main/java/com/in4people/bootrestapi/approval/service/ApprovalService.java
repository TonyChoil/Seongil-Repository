...
//        List<Approval> approvalList = approvalRepository.getApprovalList(memCode);
    ...
    public Object getBookmarkList(String memCode) {

        List<Approval> approvalList =approvalRepository.getBookmarkList(memCode);

        return approvalList.stream().map(approval -> modelMapper.map(approval, ApprovalDTO.class)).collect(Collectors.toList());

    }

    public Object getApporvalDetail(String docCode) {

        InsertApproval approvalDetail = insertApprovalRepository.findByDocCode(docCode);
